...
# the native implementation when users set SEEKY_RUST=1.
#
# Usage
...
zstd -d "$ARTIFACTS_DIR/x86_64-unknown-linux-musl/seeky-linux-sandbox-x86_64-unknown-linux-musl.zst" \
     -o "$BIN_DIR/seeky-linux-sandbox-x64"

zstd -d "$ARTIFACTS_DIR/aarch64-unknown-linux-gnu/seeky-linux-sandbox-aarch64-unknown-linux-gnu.zst" \
     -o "$BIN_DIR/seeky-linux-sandbox-arm64"

if [[ "$INCLUDE_RUST" -eq 1 ]]; then
  # x64 Linux
  zstd -d "$ARTIFACTS_DIR/x86_64-unknown-linux-musl/seeky-x86_64-unknown-linux-musl.zst" \
      -o "$BIN_DIR/seeky-x86_64-unknown-linux-musl"
  # ARM64 Linux
  zstd -d "$ARTIFACTS_DIR/aarch64-unknown-linux-gnu/seeky-aarch64-unknown-linux-gnu.zst" \
      -o "$BIN_DIR/seeky-aarch64-unknown-linux-gnu"
  # x64 macOS
  zstd -d "$ARTIFACTS_DIR/x86_64-apple-darwin/seeky-x86_64-apple-darwin.zst" \
      -o "$BIN_DIR/seeky-x86_64-apple-darwin"
  # ARM64 macOS
  zstd -d "$ARTIFACTS_DIR/aarch64-apple-darwin/seeky-aarch64-apple-darwin.zst" \
      -o "$BIN_DIR/seeky-aarch64-apple-darwin"
fi
